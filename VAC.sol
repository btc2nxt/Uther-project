...
contract VAC {
	...
	Binding: bind referees to the contract;
	...
	Finished: contractor finished its work;
	*/
		...
		returns (uint voteID)
	{
		...
		if ( (_position ==1 || _position ==4 || _position ==8) && (c.state == Contractor_State.Created || c.state == Contractor_State.Reviewing ) ) 
		{
            ...
            if (refereeId >= maxReferees ) return;

			// first review
			if (c.state != Contractor_State.Created) {
				c.state = Contractor_State.Reviewing;
				c.reviewProfessorIds[0] = 0x0;
				c.reviewProfessorIds[1] = 0x0;
			}
			...
			for (uint8 i = 0; i < professorNumber; ++i) {
				...
				//find its refereeId
				if (c.reviewProfessorIds[i*2] == byte(refereeId) ) {
					nIndex = i;
					break;
				}
			}
			...
			c.reviewProfessorIds[nIndex] = byte(refereeId);
		...
		returns (uint voteID)
	{
		...
		if ( c.state ==  Contractor_State.Binding )
		{
			...
			// first binding
			if ( c.state == Contractor_State.Binding && c.refereeIds.length ==0) {
				//c.refereeIds.length = c.requestRefereeNumber ;
				c.drawDatas.length = c.requestRefereeNumber ;
				c.refereeIds[0] = byte(refereeId);
				return;
			}
			...
			for (uint8 i = 0; i < c.requestRefereeNumber; ++i) {
				...
				//find its refereeId
				if (c.refereeIds[i] == byte(refereeId) ) {
			...
			if (nIndex != c.requestRefereeNumber)
				c.refereeIds[nIndex] = byte(refereeId);
			//all referees binded, state change to sleeping;
			if (nIndex == c.requestRefereeNumber-1)
